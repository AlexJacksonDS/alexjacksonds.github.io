...
export default function Chess() {
  ...
  const gameIdOnKeyUp = (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    if (signalRConnection) {
      signalRConnection.send("joinGame", gameId, chess.pgn()).then(() => {
  ...
  function handleClick(clickedSquare?: Square) {
    ...
    if (signalRConnection) {
      if (selectedSquare && possibleMoves && clickedSquare !== selectedSquare) {
        ...
        signalRConnection.send("takeTurn", gameId, chess.pgn());
      } else {
  ...
  function isPossiblePromotion(selectedSquare: Square) {
    ...
    return (
      (piece.type === "p" &&
        piece.color === "b" &&
        selectedSquare.charAt(1) === "2") ||
      (piece.type === "p" &&
        piece.color === "w" &&
        selectedSquare.charAt(1) === "7")
    );
  }
  ...
  function getPromotionOption() {
    ...
    while (piece == null) {
      var input = prompt(
        "Enter a piece to promote to: q = Queen, r = Rook, b = Bishop, n = Knight"
      );
    ...
    <Container>
            ...
            <ChessBoard
              ...
              previousMove={
                chess.history({ verbose: true }).length > 0
                  ? chess.history({ verbose: true }).at(-1)?.lan
                  : ""
              }
            />
          </Container>
      ...
      <Row className="black-background g-0-top text-center">
        <Col>
          ...
          {isCheckMate ? (
            <p>{`${chess.turn() === "w" ? "Black" : "White"} wins`}</p>
          ) : null}
          {isCheck && !isCheckMate ? (
            <p>{`${chess.turn() === "w" ? "White" : "Black"} is in check`}</p>
          ) : null}
