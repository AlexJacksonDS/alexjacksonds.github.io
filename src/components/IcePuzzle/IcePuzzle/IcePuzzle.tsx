...
export default function IcePuzzle() {
  ...
  const [rawSize, setRawSize] = useState(
    Math.floor(500 / (Math.max(width, height) + 2))
  );
  ...
  const drawCurrentPlayerLocation = useCallback(
    ...
    async function movePlayer(iMod: number, jMod: number) {
      ...
      while (!isRock && !isStart) {
        if (map.current[i][j] === Tile.Rock) {
          ...
          map.current[i][j] === Tile.Start ||
          map.current[i][j] === Tile.Goal
        ) {
          isStart = true;
  ...
  useEffect(() => {
    ...
    if (rect && renderMap) {
      drawCurrentPlayerLocation(
        currentJ.current * scale.current,
        currentI.current * scale.current
      );
    }
    if (!isInit) {
      ...
      if (isMobile) {
        setRawSize(
          Math.floor(window.innerWidth / ((Math.max(width, height) + 2) * 2))
        );
      }
    ...
    <>
      ...
      {renderMap ? (
        <>
          ...
          <Container className="ice-puzzle-container">
            <Container
              id="ice-puzzle"
              style={{ width: scale.current * (width + 2) + "px" }}
            >
              {map.current.map((r, i) => (
      ...
      <Container>
        <Form>
          ...
          {width >= 15 && height >= 15 ? (
            <p>
              At this size performance of generation will be impacted and map
              may be very simple/impossible. To bypass the performance timeout
              of 30 seconds select the checkbox above.
            </p>
          ) : null}
        </Form>
      </Container>
