...
export class IcePuzzleGenerator {
    ...
    while (!o && (performance.now() - start < 30000 || bypassTimeout)) {
      ...
      for (let i = 0; i < this.width + 2; i++)
        for (let j = 0; j < this.height + 2; j++)
          if (i === 0 || i === this.width + 1 || j === 0 || j === this.height + 1) this.board[i][j] = 2;

      this.start = { x: 0, y: 0 };
      let randomPerimeterDistance = Math.floor(Math.random() * (this.width * 2 + this.height * 2));
  ...
  V() {
    ...
    while (pointQueue.length > 0) {
      ...
      for (let i = 0; i < 4; i++) {
        let n = this.S(t, i < 2 ? 0 : 1, i % 2 < 1 ? -1 : 1, 99, false);
  ...
  N(): boolean {
    ...
    if (
      ...
      q.x > this.width ||
      q.y > this.height ||
      (q.x === this.current.x && q.y === this.current.y) ||
      this.board[q.x][q.y] != 0
    ) {
      return false;
    }
    let x = q.x;
  ...
  S(currentPoint: Point, direction: number, d: -1 | 1, q: number, s: boolean): Point {
    let i = 1,
      x = currentPoint.x,
      y = currentPoint.y;
    for (; i <= q; i++) {
...
export class IcePuzzleGeneratorV2 {
  width: number;
  height: number;
  directionIndex: number;
  board: number[][];
